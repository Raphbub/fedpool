...
::: {style="font-size: 80%;"}
[Code disponible sur https://github.com/Raphbub/fedpool](https://github.com/Raphbub/fedpool)
:::
...
Monter le seuil de tolérance (IoU) à 25% au lieu de 10% pour améliorer la précision et utiliser des images plus précises malgré le temps de calcul plus important
:::
...
Entraînement sur le modèle YOLOv5m

<br>

| Jeu de données         | Images | Piscines |
|--------------|-------------------:|-------------------------:|
| Entraînement | 248 (85%)          | 612 (86%)                |
| Validation   | 44 (15%)           | 99  (14%)                |

<br>

_Data augmentation_ limitée à retourner l'image (resp. horizontalement et verticalement) selon une probabilité de 0.5

## Résultats

Le modèle obtient de bons résultats après une dizaine d'_epochs_ et tend à _overfit_ ensuite

![Métriques par _epoch_](imgs/results.png)

## Résultats

::: {layout-ncol=2}
![Labels](imgs/val_batch0_labels.jpg)

![Prédits](imgs/val_batch0_pred.jpg)
:::

## Résultats

::: {layout-ncol=2}
![Labels](imgs/val_batch1_labels.jpg)

![Prédits](imgs/val_batch1_pred.jpg)
:::

## Résultats

::: {layout-ncol=2}
![Labels avec un oubli](imgs/val_batch2_labels.jpg)

![Prédits oubli détecté](imgs/val_batch2_pred.jpg)
:::

## Résultats

Le modèle a une précision plutôt correcte (rate peu de piscines "normales")

<br>

Il confond parfois avec les trampolines, toiles de tente ou toits clairs (faux positifs)

<br>

Peine à distinguer les piscines bâchées avec peu de contraste

<br>

Au vu des prédictions, on pourrait mettre une limite à 0.6-0.7 pour restreindre le nombre de faux positifs
...
::: {.fragment}
Augmenter le nombre d'images ou séparer en multiples classes ?
:::
...
![Mes attentes | En vrai, un batch de validation](imgs/v1_val_batch0_labels.jpg)

------------------------------------------------------------------------

![La réalité | Prédictions effectives du même batch](imgs/v1_val_batch0_pred.jpg)
